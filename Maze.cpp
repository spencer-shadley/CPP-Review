...
class Maze {
	...
	friend Maze& operator=(const Maze& lhs, const Maze& rhs) {
		lhs.room = rhs.room; // will this just use pointer equality? Do I need to dereference like this -> [    *(lhs.room) = *(rhs.room);     ] ?
		lhs.door = rhs.door;
	}
...
public:
	void increase_volume(size_t amount) {
		volume += amount;
	}
